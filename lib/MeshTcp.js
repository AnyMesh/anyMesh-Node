...
function MeshTcpHandler(anyMesh, startingPort) {
    ...
    this.tcp_server = net.createServer(function (socket) {
        ...
        if(self.connectionExists(socket.ipAddress, socket.port)) {
            socket.destroy();
            return;
        }
...
MeshTcpHandler.prototype.addConnection = function (address, port) {
    //connect to remote tcp server
    var self = this;

    if(!this.connectionExists(address, port)) {
...
MeshTcpHandler.prototype.connectionExists = function(address, port) {
    for(var index in connections) {
        var connection = connections[index];
        if (connection.ipAddress == address && connection.port == port) return true;
    }
    return false;
}
