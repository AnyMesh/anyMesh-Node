...
AnyMesh.prototype._processSystemMessage = function(socket, msgObj) {
    ...
    if (sysData.type = AnyMesh.MsgTypeSystem.SUBSCRIPTIONS) {
        if (sysData.isUpdate) {
            ...
            this.tcpUpdatedSubscriptions(socket.info.subscriptions, socket.info.name);
        }
        else if(!this.socketForName(msgObj.sender)){
            ...
            socket.destroy();
            this.connections.splice(self.connections.indexOf(socket), 1);
        }
    }
};
