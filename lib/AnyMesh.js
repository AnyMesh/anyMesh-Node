...
MeshUdp.prototype.startBroadcasting = function() {
    ...
    this.timerId = setInterval(function () {
        var buffer = new Buffer(self.networkID + ',' + self.tcpPort + ',' + self.anyMesh.name);
    ...
    if(!this.socketForName(name)) {
        ...
        newServer.connect(port, address, function() {
            ...
            newServer.on('end', function (){
                ...
                self.tcpDisconnected(newServer);
            });
            ...
            my_carrier.on('line', function (package) {
                ...
                if (msgObj.type == "info") {
                    ...
                    if(existingSocket) {
                        //compare indexes... disconnect and return if we are the last           GETTING RID OF THIS!!
